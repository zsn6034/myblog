var sqlMap = {
  article: {
    ...
    queryAll: 'SELECT SQL_CALC_FOUND_ROWS a.*, GROUP_CONCAT(DISTINCT(atj.tag_name)) tags from articles a LEFT JOIN article_tag_join atj ON a.title=atj.article_title WHERE state =? GROUP BY id ORDER BY post_time desc limit ?, ?;SELECT FOUND_ROWS() as count',
    ...
    queryAllBysu: 'SELECT SQL_CALC_FOUND_ROWS a.*, GROUP_CONCAT(DISTINCT(atj.tag_name)) tags from articles a LEFT JOIN article_tag_join atj ON a.title=atj.article_title WHERE state=? AND username=? GROUP BY id ORDER BY post_time desc limit ?, ?;SELECT FOUND_ROWS() as count',
    // queryAllBySU: 'SELECT * from articles WHERE state =? AND username =?',
    queryAllBySU: 'SELECT a.*, GROUP_CONCAT(DISTINCT(atj.tag_name)) tags from articles a LEFT JOIN article_tag_join atj ON a.title=atj.article_title WHERE state =? AND username =? GROUP BY id',
    ...
    insertArticleTagJoin: 'INSERT INTO article_tag_join(article_title, tag_name) VALUES (?,?)',
