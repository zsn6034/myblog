var sqlMap = {
  article: {
    ...
    queryById1: 'SELECT title FROM articles WHERE id=?',
    queryById2: 'SELECT a.*, GROUP_CONCAT(DISTINCT(t.name)) tags from articles a LEFT JOIN article_tag_join atj ON a.id=atj.article_id LEFT JOIN tags t on atj.tag_id=t.id WHERE a.id=? GROUP BY a.id ORDER BY post_time',
    queryByTitle: 'SELECT title FROM articles WHERE title=?',
    delById: 'DELETE FROM articles WHERE id=?',
    delJoin: 'DELETE FROM article_tag_join WHERE article_id=?',
    updById: 'UPDATE articles SET state=? WHERE id = ?',
    // updAllById: 'UPDATE articles SET state=?,title=?,type=?,summary=? WHERE id = ?',
    updArticleById: 'UPDATE articles SET state=?,title=?,summary=? WHERE id = ?',
    delArticleTagJoin: 'DELETE FROM article_tag_join WHERE article_title=?',
    // queryAll: 'SELECT SQL_CALC_FOUND_ROWS * from articles WHERE state =? order by post_time desc limit ?, ?;SELECT FOUND_ROWS() as count',
    queryAll: 'SELECT SQL_CALC_FOUND_ROWS a.*, GROUP_CONCAT(DISTINCT(t.name)) tags from articles a LEFT JOIN article_tag_join atj ON a.id=atj.article_id LEFT JOIN tags t on atj.tag_id=t.id WHERE state =? GROUP BY id ORDER BY post_time desc limit ?, ?;SELECT FOUND_ROWS() as count',
    ...
    queryAllBysu: 'SELECT SQL_CALC_FOUND_ROWS a.*, GROUP_CONCAT(DISTINCT(t.name)) tags from articles a LEFT JOIN article_tag_join atj ON a.id=atj.article_id LEFT JOIN tags t on atj.tag_id=t.id WHERE state=? AND username=? GROUP BY a.id ORDER BY post_time desc limit ?, ?;SELECT FOUND_ROWS() as count',
    // queryAllBySU: 'SELECT * from articles WHERE state =? AND username =?',
    queryAllBySU: 'SELECT a.*, GROUP_CONCAT(DISTINCT(t.name)) tags from articles a LEFT JOIN article_tag_join atj ON a.id=atj.article_id LEFT JOIN tags t on atj.tag_id=t.id WHERE state=? AND username=? GROUP BY a.id',
    ...
    queryMaxIdPlusOne: 'SELECT IFNULL(MAX(CAST(id AS UNSIGNED)),0) + 1 newId FROM articles',
    insertNewArticle: 'INSERT INTO articles(id, username, title,summary, shtml, post_time, view, comment_count,state) VALUES (?,?,?,?,?,?,?,?,?)',
    insertArticle: 'INSERT INTO articles(username, title,summary, shtml, post_time, view, comment_count,state) VALUES (?,?,?,?,?,?,?,?)',
    insertArticleTagJoin: 'INSERT INTO article_tag_join(article_id, tag_id) VALUES (?,?)',
